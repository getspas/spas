...
func TestSyncRejectsUnexpectedCleanPrivateHead(t *testing.T) {
	...
	runGit(t, private.Path, "config", "user.name", "SPAS Test")
	runGit(t, private.Path, "config", "user.email", "spas@example.invalid")
...
func TestPublicTrackedNonPortableNamesDoNotBreakCommands(t *testing.T) {
	...
	ctx := context.Background()
	instance, publicRoot, _, _ := fixture(t)
	runGit(t, publicRoot, "config", "--local", "core.protectNTFS", "false")
